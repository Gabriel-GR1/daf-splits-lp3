...
// Mantém a lógica complementar da página isolada do app.js para evitar
// colisões de variáveis globais que possam interromper o formulário.
(() => {
  const successStateEl = document.getElementById('successState');

  if (successStateEl && !successStateEl.querySelector('.success-group-cta')) {
    const successCtaEl = document.createElement('div');
    successCtaEl.className = 'success-group-cta';
    successCtaEl.innerHTML = `
    ...
    const confirmationEl = successStateEl.querySelector('.success-confirmation');
    if (confirmationEl) confirmationEl.insertAdjacentElement('afterend', successCtaEl);
    else successStateEl.appendChild(successCtaEl);

    const successCtaStyleEl = document.createElement('style');
    successCtaStyleEl.textContent = `
      .success-group-cta {
        width: min(390px, 100%);
  ...
  appScriptEl.src = './app.js';
  appScriptEl.async = false;
